...
var readerApp = {
  ...
  setFancyBoxVideo: function () {
    ...
    $(".video-fancybox").click(function (e) {
      ...
      var viewportWidth = Math.min(window.innerWidth * 0.9, 900);
      var aspectRatio = 9 / 16; // 16:9 aspect ratio
      var calculatedHeight = Math.floor(viewportWidth * aspectRatio);
      ...
      if (
        ...
        var iSize = window.fancyboxSize.split("x");
        // Use aspect ratio from provided size
        aspectRatio = parseInt(iSize[1]) / parseInt(iSize[0]);
        calculatedHeight = Math.floor(viewportWidth * aspectRatio);
        window.fancyboxSize = "";
      }
      
      fancyVidDim.height = calculatedHeight + "px";
